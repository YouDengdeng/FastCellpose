...
from .models import models_logger
...
class UnetModel():
    def __init__(self, gpu=False, pretrained_model=False,
        ...
        self.nbase = [16, 32, 64, 128]
        self.nchan = nchan
        # self.nbase = [nchan, 32, 64, 128, 256]
        self.nbase = [nchan, 16, 32, 64, 128]
    ...
    def _run_net(self, imgs, augment=False, tile=False, tile_overlap=0.1, bsize=256,
                 return_conv=False):
...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
    ...
    def train(self, train_data, train_labels, train_files=None, 
              ...
              learning_rate=0.2, n_epochs=210, momentum=0.9, weight_decay=0.00001, batch_size=8,
              nimg_per_epoch=None, min_train_masks=1, rescale=False, model_name=None):
    ...
    # 每个epoch之间的输出
    def predict_and_evaluate(self, iepoch, test_while_train_root):
        # test_img = np.zeros((2, 1024, 1024), np.float32)
        test_img = []
        test_lbl = []
        test_img_path = test_while_train_root + r'input/'
        test_names = os.listdir(test_img_path)
        test_names1 = os.listdir(test_while_train_root + r'mask/')
        n_img = len(test_names)
        for n in range(n_img):
            name = test_while_train_root + r'mask/' + test_names1[n]
            temp = cv2.imread(test_img_path + test_names[n], 0)
            temp1 = cv2.imread(name, 0)
            temp1[temp1 > 0] = 1
            test_img.append(temp)
            test_lbl.append(temp1)
        ...
        for i in iterator:
            img = np.asarray(test_img[i])
            img = transforms.normalize_img(img, invert=False)
            y, style = self._run_net(img)
            cellprob[i] = y[:, :, 2]
            dP[:, i] = y[:, :, :2].transpose((2, 0, 1))
            outputs = dynamics.compute_masks(dP[:, i], cellprob[i], niter=200,
                                             cellprob_threshold=0.0,
                                             flow_threshold=0.4, interp=True, resize=resize,
        ...
        for m in range(n_img):
            test_name = test_names[m].split('.')[0]
            os.makedirs(test_while_train_root + r'predict_results/', exist_ok=True)
            file_name = test_while_train_root + r'predict_results/' + test_name + '_epoch%d.png' % iepoch
            cv2.imwrite(file_name, masks[m])
            dice.append(dice_coef(masks[m], test_lbl[m]))
            iou.append(iou_score(masks[m], test_lbl[m]))
        dice = np.mean(dice)
        iou = np.mean(iou)
        return dice, iou
    ...
    def _train_net(self, train_data, train_labels, 
              ...
              save_path=None, test_while_train_root=None,
              custom_path=None, save_every=100, save_each=False,
              learning_rate=0.2,start_epoch=None, n_epochs=1000, momentum=0.9, weight_decay=0.00001,
        ...
        if isinstance(learning_rate, (list, np.ndarray)):
            ...
            if SGD:
                ...
                if self.n_epochs > 50:
                    LR = np.append(LR, self.learning_rate_const*np.ones(self.n_epochs-60))
                    for i in range(5):
            ...
            #         LR = np.append(LR, self.learning_rate_const*np.ones(self.n_epochs-110))
            #         # LR = np.append(LR, self.learning_rate_const * np.ones(self.n_epochs - 310))
            #         for i in range(10):
            #             LR = np.append(LR, LR[-1]/2 * np.ones(10))
            #     else:
            #         LR = np.append(LR, self.learning_rate_const*np.ones(max(0, self.n_epochs-10)))
        ...
        self.batch_size = batch_size
        self._set_optimizer(self.learning_rate[start_epoch], momentum, weight_decay, SGD)
        self._set_criterion()
        ...
        if save_path is not None and custom_path==0:
            _, file_label = os.path.split(save_path)
            file_path = os.path.join(save_path, 'models/')
            log_path = os.path.join(save_path, 'log/')
            os.makedirs(file_path, exist_ok=True)
            os.makedirs(log_path, exist_ok=True)
        elif save_path is not None and custom_path == 1:
            file_path = save_path
        ...
        for iepoch in range(start_epoch+1, self.n_epochs):
            if SGD:
                self._set_learning_rate(self.learning_rate[iepoch])
            np.random.seed(iepoch)
            rperm = inds_all[iepoch*nimg_per_epoch:(iepoch+1)*nimg_per_epoch]
            nsum = 0
            #for ibatch in trange(0,nimg_per_epoch,batch_size):
            for ibatch in tqdm(range(0,nimg_per_epoch,batch_size)):
                ...
                lavg += train_loss
                nsum += len(imgi)
            dice, iou = self.predict_and_evaluate(iepoch, test_while_train_root)
            ...
            if test_data is not None:
                ...
                core_logger.info('Epoch %d, Time %4.1fs, Loss %2.4f, Loss Test %2.4f, LR %2.4f. DICE %2.4f, IOU %2.4f'%
                        (iepoch, time.time()-tic, lavg, lavgt/nsum, self.learning_rate[iepoch],dice, iou))

                with open(log_path + 'log_test.txt', 'a') as f:
                    f.write('Epoch : {}, loss: {:4f}, loss test: {:4f}, LR {:5f}, DICE {:4f}, IOU {:4f}\n'.format(iepoch + 1,
                                                                                           lavg,
                                                                                           lavgt/nsum,
                                                                                           self.learning_rate[iepoch],dice,iou))
                with open(log_path + 'log.txt','a') as f:
                    f.write('%2.4f %2.4f %2.4f\n' %(lavg, lavgt/nsum, self.learning_rate[iepoch]))
            else:
            ...
            if save_path is not None:
                    ...
                    if save_each: #separate files as model progresses 
                        if model_name is None:
                            ...
                            #                                 'epoch_'+str(iepoch),'.pth')
                            file_name = os.path.join(self.net_type+
                                                      'epoch_'+str(iepoch)+'.pth')
                        else:
                            file_name = '{}_{}'.format(model_name, 'epoch_'+str(iepoch))
                    else:
                        if model_name is None:
                            #file_name = '{}_{}_{}'.format(self.net_type, file_label, d.strftime("%Y_%m_%d_%H_%M_%S"))
                            file_name = os.path.join(self.net_type+'_epoch_' + str(iepoch)+'.pth')
                        else:
                            file_name = '{}_{}.pth'.format(model_name, 'epoch_'+str(iepoch))
