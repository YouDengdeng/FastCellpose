...
def follow_flows(dP, mask=None, niter=200, interp=True, use_gpu=True, device=None):
    ...
    if len(shape) > 2:
        ...
        p = np.meshgrid(np.arange(shape[0]), np.arange(shape[1]), indexing='ij')  # ij表示离散的，xy表示笛卡尔坐标系
        p = np.array(p).astype(np.float32)

        inds = np.array(np.nonzero(np.abs(dP[0]) > 1e-3)).astype(np.int32).T  # 得到的是有效信号的坐标值，inds维度为：行数X2
...
def compute_masks(dP, cellprob, p=None, niter=200, # dP[:, j]:2X256X256，为第j张图的两个向量图，cellrob:256X256
    ...
    if np.any(cp_mask):  # mask at this point is a cell cluster binary map, not labels
        # follow flows
        if p is None:
            p, inds = follow_flows(dP * cp_mask / 5., niter=niter, interp=interp,   # inds包含了一些坐标，p为meshgrid，其中对应inds坐标的值做了修改
