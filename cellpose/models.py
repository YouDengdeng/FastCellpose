...
class Cellpose():
    ...
    def __init__(self, model_type=None, gpu=False, pretrained_model=None,
                 net_avg=False, device=None, pretrained_size=None):
        # model_type='cyto'
    ...
    def eval(self, x, batch_size=8, channels=None, channel_axis=None, z_axis=None,
             ...
             rescale=1, progress=None, model_loaded=False):
        ...
        if estimate_size and self.pretrained_size is not None and not do_3D and x[0].ndim < 4:
            ...
            diams, _ = self.sz.eval(x, channels=channels, channel_axis=channel_axis, invert=invert,
...
class CellposeModel(UnetModel):
    ...
    def __init__(self, gpu=False, pretrained_model=False,
        ...
        if model_type is not None or (pretrained_model and not os.path.exists(pretrained_model[0])):
            ...
            if pretrained_model is None:
                models_logger.info(f'>>>>no model loaded, training from scratch')
        ...
        if self.pretrained_model is None:
            models_logger.info(
                f'>>>> unchanged model diam_mean = {self.diam_mean: .3f} (ROIs rescaled to this size during training)')
        ostr = ['off', 'on']
        self.net_type = 'cellpose_'

    def eval(self, x, batch_size=8, channels=None, channel_axis=None,
             z_axis=None, normalize=True, invert=False,
             rescale=1, diameter=None, do_3D=False, anisotropy=None, net_avg=False,
                ...
                maski, flowi, stylei = self.eval(x[i],
                                                 ...
                                                 channels=channels[i] if (len(channels) == len(x) and
                                                                          (isinstance(channels[i], list) or isinstance(
                                                                              channels[i], np.ndarray)) and
                                                                          len(channels[i]) == 2) else channels,
                                                 channel_axis=channel_axis,
                                                 z_axis=z_axis,
                                                 normalize=normalize,
                                                 invert=invert,
                                                 rescale=rescale[i] if isinstance(rescale, list) or isinstance(rescale,
                                                                                                               np.ndarray) else rescale,
                                                 diameter=diameter[i] if isinstance(diameter, list) or isinstance(
                                                     diameter, np.ndarray) else diameter,
                                                 do_3D=do_3D,
                                                 anisotropy=anisotropy,
                                                 net_avg=net_avg,
                                                 augment=augment,
    ...
    def train(self, train_data, train_labels, train_files=None,
              ...
              test_while_train_root=None,
              channels=None, normalize=True,
              save_path=None, save_every=20, save_each=False,
              learning_rate=0.2, n_epochs=500, momentum=0.9, SGD=True,
              weight_decay=0.00001, batch_size=16, nimg_per_epoch=None,
              rescale=True, min_train_masks=0,
              model_name=None, start_epoch=None):
        ...
        model_path = self._train_net(train_data=train_data, train_labels=train_labels,
                                     test_data=test_data, test_labels=test_labels,
                                     save_path=save_path, test_while_train_root=test_while_train_root,
                                     custom_path=0, save_every=save_every, save_each=save_each,
                                     learning_rate=learning_rate, start_epoch=start_epoch, n_epochs=n_epochs,
...
class SizeModel():
    ...
    def eval(self, x, channels=None, channel_axis=None,
             ...
             batch_size=16, progress=None, interp=True):
            ...
            for i in iterator:
                diam, diam_style = self.eval(x[i],
                                             channels=channels[i] if (
                                                         ...
                                                         (isinstance(channels[i], list) or isinstance(channels[i],
                                                                                                      np.ndarray)) and
                                                         len(channels[i]) == 2) else channels,
                                             channel_axis=channel_axis,
                                             normalize=normalize,
    ...
    def train(self, train_data, train_labels,
              ...
              l2_regularization=1.0, batch_size=16,
              ):
        ...
        if isinstance(self.cp.pretrained_model, list):
            ...
            cp_model_path = self.cp.model_name
        ...
        for iepoch in range(n_epochs):
            ...
            for ibatch in tqdm(range(0, nimg, batch_size)):
